...
CREATE TABLE IF NOT EXISTS `mydb`.`venda_itens_estoque` (
  ...
  `qtd` INT NOT NULL,
  INDEX `fk_venda_has_estoque_vendas1_idx` (`vendas_idvend` ASC) VISIBLE,
  INDEX `fk_venda_has_estoque_estoque1_idx` (`estoque_idest` ASC) VISIBLE,
  PRIMARY KEY (`vendas_idvend`),
...
DELIMITER $$
USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`compra_BEFORE_INSERT` BEFORE INSERT ON `compra` FOR EACH ROW
BEGIN
